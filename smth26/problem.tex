...
\begin{figure}[t]
    ...
    iterations of the BE tasks}\label{fig:srv-bg-unedited}
\end{figure}
...
them to the same set of cores.\hmng{we could do an initial version where we run
with weights from Kuberentes, and then use the bigger split to show that doens't
solve it? The 10000 feels like an escalation, now that I'm saying the weight in
the intro} \autoref{fig:srv-bg-unedited} shows the result: we see that at a
...
and much higher for 99th percentile latencies. At a baseline utilization of
95\%, those numbers increase to up to 40ms for the the mean latency, and 80ms
for the tail.
