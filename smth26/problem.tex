...
local property}. Linux maintains a separate run-queue on each core, and within
...
The last experiment underscores the problems identified in this
previous subsections are unrelated to Kubernetes.  The last experiment
above doesn't involve Kubernetes, just \cgroups{}.

\subsection{Alternatives to cgroups}
...
weight is only effective within the \normalclass{} class.  In the
appendix we show that the \schedidle{} policy for the \normalclass{}, and
other scheduling classes are not satisfactory alternatives to
\cgroups{}.

\subsection{Summary}

From this section we conclude that \cgroups{} doesn't deliver on what
the documentation says: each group should get time proportional to its
weight as a share of the sum of weights of runnable groups.  The
reason Linux fails to achieve this goal is because Linux uses per-CPU
run-queues, and enforces weights only per run-queue, violating what the
documentation says. The per-core weights result in scheduling
anonamolies where one core doesn't realize that another core has a
runnable high-weight process.
