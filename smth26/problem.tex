...
In order to isolate why the jump in latency is happening when the BEs run, we
...
runqueue on each core, in order to avoid the overheads of accessing global state
...
weight one, whereas another core has queued high weight processes.
