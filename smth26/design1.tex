...
virtual time globally, and, handle multiple cores
concurrently assigning virtual times to processes of the same group.
In Linux virtual time is per run queue (and not global), and updated
only by one core. As a result, Linux, for example, doesn't need to
update the group's virtual on enqueue, doesn't need to adjust it if a
process runs shorter than its full tick, and doesn't need to set a
group's virtual time when it wakes up.

\subsection{\mheap: an approximate global heap}
\label{sec:approximate}

\sys's heap of global virtual time is written by many cores
concurrently and often.  Cores select the process with the minimal
virtual time from the heap and cores insert runnable processes in the
heap based on their virtual times.
