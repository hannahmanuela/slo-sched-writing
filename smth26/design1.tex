...
virtual time from the heap and insert runnable processes in the heap
based on their virtual times.  To avoid serializing the writes to the
global heap, \sys uses a multi-heap design with an relaxed global
minimum, which we call \mheap, and is based on fast relaxed concurrent
priority queues~\cite{}.
